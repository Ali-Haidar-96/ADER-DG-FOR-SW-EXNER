...
\title{ADER--DG Formulation for the Exner Model in the Low-Froude Regime}
\author{}
\date{}
...
\section{The shallow-water--Exner model in the low-Froude regime}

We consider the one-dimensional shallow-water--Exner model, with the
shallow-water part written in the low-Froude-number regime. Let
\(\mathrm{Fr}>0\) denote the Froude number, defined from characteristic velocity
and water-depth scales by
\[
    \mathrm{Fr}=\frac{U_{\mathrm{ref}}}{\sqrt{gH_{\mathrm{ref}}}}.
\]
In the following, we are interested in the regime \(\mathrm{Fr}\ll 1\). With the
standard nondimensional low-Froude scaling, the gravitational terms in the
shallow-water momentum equation are multiplied by \(1/\mathrm{Fr}^2\). The
Exner equation and the sediment-transport law are kept unchanged.

Let \(h=h(x,t)\) denote the water height, \(u=u(x,t)\) the depth-averaged water
velocity, \(q=q(x,t)=h(x,t)u(x,t)\) the water discharge, and \(b=b(x,t)\) the
evolving bottom topography. The free-surface elevation is
\[
    \eta=h+b,
\]
so that
...
\[
    \partial_t q
    +\partial_x\!\left(
        qu+\frac{1}{2\mathrm{Fr}^2}\bigl(\eta^2-2\eta b\bigr)
    \right)
    =-\frac{\eta}{\mathrm{Fr}^2}\,\partial_x b.
\]
Consequently, using the variables \((\eta,q,b)\), the shallow-water--Exner system takes the
compact form
\begin{equation}
...
\[
    ...
    \begin{pmatrix}
        q+q_b\\[1mm]
        qu+\dfrac{1}{2\mathrm{Fr}^2}\bigl(\eta^2-2\eta b\bigr)\\[1mm]
        q_b
    \end{pmatrix},
    \qquad
    \bm S=
    \begin{pmatrix}
        0\\[1mm]
        -\dfrac{\eta}{\mathrm{Fr}^2}\,\partial_x b\\[1mm]
        0
    \end{pmatrix}.
\]
...
For the low-Froude shallow-water--Exner system considered here, the transformed flux and source
are decomposed as
\begin{equation}
\left\{
\begin{aligned}
...
\begin{align*}
    ...
    &=
    \frac{1}{2\mathrm{Fr}^2}\widetilde{(\eta^2)}(k_x,k_t)                       \\
    &=
    ...
    &=
    \frac{1}{\mathrm{Fr}^2}\widetilde{(\eta b)}(k_x,k_t)                              \\
    &=
    \frac{1}{\mathrm{Fr}^2}
    \sum_{r=0}^{k_x}\sum_{s=0}^{k_t}
    \widetilde{\eta}(r,s)
...
\begin{align*}
    ...
    &=
    \frac{1}{\mathrm{Fr}^2}\widetilde{(\eta b_x)}(k_x,k_t)                             \\
    &=
    \frac{1}{\mathrm{Fr}^2}
    \sum_{r=0}^{k_x}\sum_{s=0}^{k_t}
    \widetilde{\eta}(r,s)
